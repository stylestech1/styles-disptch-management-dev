...
type RoleFilter = "all" | "employee" | "driver" | "admin" | "manager";
const Users = () => {
  ...
  const users = useMemo(() => {
    ...
    if (isFiltered && Array.isArray(filteredData?.data)) {
  ...
  useEffect(() => {
    const currentError = dispatchersError || userByIdError || filteredError || roleError;
  ...
  }, [dispatchersError, userByIdError, filteredError, setError, roleError]);
    ...
    <Box sx={containerSx}>
      ...
      <DataTable
        ...
        loading={Boolean(
          ...
          (isFiltered && filteredLoading) ||
          (roleFilter !== "all" && roleLoading) ||
      ...
      {!isFiltered && !isSearching && pagination && users.length > 0 && (
        <Pagination
